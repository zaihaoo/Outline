const urls = ["./shader/ColorfulTime.vs", "./shader/ColorfulTime.fs", "./shader/GaussianBlur.vs", "./shader/GaussianBlur.fs"];
Promise.all(urls.map(url =>
	...
	const selectionMask = _createTexture();
	const sobelMask = _createTexture();
...
`;
    ...
    const sobelShader = createSobelShader(gl);
    ...
    function render(nowMSec) {
		...
		gl.uniform1i(sobelShader.textureLoc, 0);  
		gl.uniform2fv(sobelShader.pixelSizeLoc, pixelSize);
		gl.uniform4fv(sobelShader.colorLoc, OUTLINE_COLOR);
        gl.uniform1f(sobelShader.sizeLoc, Math.max(gl.drawingBufferWidth,gl.drawingBufferHeight));   
        gl.uniform1f(sobelShader.time, now * 0.6 / 60);
		...
		gl.disable(gl.BLEND);
		gl.enable(gl.DEPTH_TEST);
		gl.disableVertexAttribArray(sobelShader.verticesLoc);
        ...
        gl.bindTexture(gl.TEXTURE_2D, sobelMask);
		...
		gl.uniform1i(blurShader.textureLoc, 0);  
		gl.uniform2fv(blurShader.pixelSizeLoc, pixelSize);
		gl.uniform4fv(blurShader.colorLoc, OUTLINE_COLOR);
        gl.uniform2fv(blurShader.sizeLoc, [gl.drawingBufferWidth,gl.drawingBufferHeight]);   
