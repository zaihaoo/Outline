const urls = ["./shader/off_screen.vs", "./shader/off_screen.fs"];
...
`;
    ...
    function render(nowMSec) {
        ...
        // 重新给framebuffer绑定新的texture 用来接收灰度贴图
	    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, grayMask, level);
        gl.bindTexture(gl.TEXTURE_2D, selectionMask);
		gl.useProgram(grayShader.program);
		gl.bindBuffer(gl.ARRAY_BUFFER, quad.vertices);
		gl.vertexAttribPointer(grayShader.verticesLoc, 2, gl.FLOAT, false, 0, 0);
		gl.enableVertexAttribArray(grayShader.verticesLoc);
		gl.uniform1i(grayShader.textureLoc, 0);  // first texture
		gl.uniform4fv(grayShader.colorLoc, OUTLINE_COLOR);
        gl.uniform1f(grayShader.sizeLoc, Math.max(gl.drawingBufferWidth,gl.drawingBufferHeight));   
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
        ...
        // 必须读取帧缓存中的数据
        // var pixels = new Float32Array(16);
        // gl.readPixels(450, 450, 2, 2, gl.RGBA, gl.FLOAT, pixels);
        // console.log(pixels);
